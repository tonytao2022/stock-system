...
SEASON_PARAMS = {
    'summer':        {'buy':74, 'hold':30, 'stop':0.12, 'take':0.18, 'sp':50, 'tp':50},
    'spring':        {'buy':74, 'hold':30, 'stop':0.12, 'take':0.15, 'sp':35, 'tp':40},
    'weak_spring':   {'buy':74, 'hold':25, 'stop':0.11, 'take':0.15, 'sp':35, 'tp':40},
    'chaos_spring':  {'buy':74, 'hold':25, 'stop':0.11, 'take':0.15, 'sp':20, 'tp':35},
    'chaos':         {'buy':74, 'hold':25, 'stop':0.10, 'take':0.12, 'sp':20, 'tp':30},
    'chaos_autumn':  {'buy':74, 'hold':20, 'stop':0.08, 'take':0.10, 'sp':15, 'tp':20},
    'weak_autumn':   {'buy':74, 'hold':20, 'stop':0.08, 'take':0.12, 'sp':20, 'tp':25},
    'autumn':        {'buy':74, 'hold':20, 'stop':0.10, 'take':0.12, 'sp':30, 'tp':35},
    'winter':        {'buy':74, 'hold':10, 'stop':0.05, 'take':0.08, 'sp':5,  'tp':10},
}
...
def main():
    ...
    cur.execute("""
        ...
        WHERE alpha062_score IS NOT NULL
    """)
    for r in cur.fetchall():
        key = (r['ts_code'], str(r['trade_date']))
        alpha_data[key] = {'a062': float(r['alpha062_score'] or 50), 'a046': float(r['alpha046_score'] or 50)}
    cur.close()
    print(f"  Alpha因子: {len(alpha_data)}条")
    
    # ====== A062_WEIGHT 和 A046_GATE 配置（可被环境变量覆盖） ======
    A062_WEIGHT = float(os.environ.get('BT_A062_WEIGHT', '0.0'))
    A046_GATE = os.environ.get('BT_A046_GATE', 'False') == 'True'
    A046_GATE_MIN = int(os.environ.get('BT_A046_GATE_MIN', '30'))
    ...
    for r in scores:
        ...
        if A062_WEIGHT > 0:
            ak = (code, td)
            if ak in alpha_data:
                a062 = alpha_data[ak]['a062']
                m1 = m1 * (1 - A062_WEIGHT) + a062 * A062_WEIGHT
        
        # alpha046门控
        if A046_GATE:
            ak = (code, td)
            if ak in alpha_data:
                a046 = alpha_data[ak]['a046']
                if a046 < A046_GATE_MIN:
                    continue
        
        day_scores.setdefault(td, []).append({
            'ts_code': code, 'score': m1
        })
...
def run_backtest(conn, day_scores, all_dates):
    ...
    for di, td in enumerate(all_dates):
        ...
        if len(positions) < 5:  # 最多5只，季节仓位控制单票比例
            ...
            slots = min(5 - len(positions), 3)  # 最多一天买3只
    ...
    print(f"\n\n{'='*55}")
    print(f"  📊 V13.2 季节参数 · 74买入线 · +补仓+止盈")
