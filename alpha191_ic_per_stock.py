#!/usr/bin/env python3
"""
Alpha191 因子 IC — 每只股票独立SQL查询
========================================
849只 × 每次查询几百行，传输快，内存低
28个Alpha因子，5日RankIC
"""
...
def compute_alphas_numpy(c, o, h, l, v, tds):
    """numpy批量计算所有因子"""
    n = len(c)
    if n < 30: return {}
    results = {}
    for i in range(29, n):
        ...
        try:
            ...
            if i>=5 and np.std(v[i-5:i+1][:5])>0:
...
def main():
    ...
    # 构建日期-收盘索引—只读trade_date, ts_code, close（轻量查询）
    print(f"⏳ 构建日期索引...", end='', flush=True)
    conn = pymysql.connect(host='127.0.0.1', port=3306, user='debian-sys-maint',
                           password=pwd, database='stock_db_v2', charset='utf8mb4')
    cur = conn.cursor()
    cur.execute(f"""
    ...
    for r in cur.fetchall():
        td = r[0].strftime('%Y-%m-%d')
        date_close[td][r[1]] = float(r[2])
        cnt += 1
    conn.close()
    alld = sorted(date_close.keys())
    print(f" {len(alld)}交易日, {cnt}条 ({time.time()-t0:.0f}s)")
    ...
    for idx, code in enumerate(codes):
        ...
        tds = [r[0].strftime('%Y-%m-%d') for r in klines]
        c = np.array([float(r[4]) for r in klines])  # close
        o = np.array([float(r[1]) for r in klines])  # open
        h = np.array([float(r[2]) for r in klines])  # high
        l = np.array([float(r[3]) for r in klines])  # low
        v = np.array([float(r[5]) for r in klines])  # vol
        
        result = compute_alphas_numpy(c, o, h, l, v, tds)
    ...
    fp = f'/tmp/alpha191_ic_v7_{time.strftime("%Y%m%d_%H%M%S")}.json'
    with open(fp, 'w') as f:
