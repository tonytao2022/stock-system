#!/usr/bin/env python3
"""
Alpha191 因子 IC v11 — 从预生成压缩文件加载数据
===============================================
849只 × 28因子 × 5日RankIC
"""
...
def main():
    ...
    for line in gzip.open('/tmp/dates_index.tsv.gz', 'rt'):
        parts = line.strip().split('\t')
        if len(parts) < 3: continue
        td, code = parts[0], parts[1]
        date_close[td][code] = pf(parts[2])
    alld = sorted(date_close.keys())
    tot_rows = sum(len(v) for v in date_close.values())
    print(f" {len(alld)}交易日, {tot_rows}条 ({time.time()-t0:.0f}s)")
    
    # 2. 获取股票列表
    print(f"⏳ 加载股票列表...", end='', flush=True)
    import subprocess
    cmd = ['mysql', '-u', 'debian-sys-maint', f'-p{PWD}', 'stock_db_v2', '-N', '-B']
    r = subprocess.run(cmd + ['-e', 'SELECT ts_code FROM backtest_pool'],
                       capture_output=True, text=True, timeout=10)
    codes = [l.strip() for l in r.stdout.strip().split('\n') if l.strip()]
    print(f" {len(codes)}只")
    ...
    for idx, code in enumerate(codes):
        ...
        for line in lines:
            ...
            tds.append(parts[0])
            o.append(pf(parts[1])); h.append(pf(parts[2]))
            l.append(pf(parts[3])); c.append(pf(parts[4])); v.append(pf(parts[5]))
    ...
    fp = f'/tmp/alpha191_ic_v11_{time.strftime("%Y%m%d_%H%M%S")}.json'
    with open(fp, 'w') as f:
