#!/usr/bin/env python3
"""
Alpha191 因子 IC v9 — mysql CLI 分批加载（无反引号冲突）
=========================================
849只 × 28因子 × 5日RankIC
"""
...
def run_sql(sql_text):
    """执行SQL，返回行列表（每行列列表）"""
    # 用stdin传递SQL避免shell解释反引号
    p = subprocess.run(
        ['mysql', '-u', 'debian-sys-maint', f'-p{PWD}', 'stock_db_v2', '-N', '-B'],
        input=sql_text, capture_output=True, text=True, timeout=30
    )
...
def main():
    ...
    # 1. 股票列表
    rows = run_sql("SELECT ts_code FROM backtest_pool")
    ...
    fp = f'/tmp/alpha191_ic_v9_{time.strftime("%Y%m%d_%H%M%S")}.json'
    with open(fp, 'w') as f:
