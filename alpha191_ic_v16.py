#!/usr/bin/env python3
"""
Alpha191 因子 IC — 终极版：文件管道+stdin子进程
=================================================
"""
...
def main():
    ...
    # 第一步：预先生成必要的中间文件
    print(f"⏳ 导出日期索引...", end='', flush=True)
    sql = f"SELECT trade_date, ts_code, close FROM daily_kline WHERE trade_date>='{START}' AND trade_date<='{END}' ORDER BY trade_date"
    r = subprocess.run(
        ['mysql', '-u', 'debian-sys-maint', f'-p{PWD}', 'stock_db_v2', '-N', '-B'],
        input=sql, capture_output=True, text=True, timeout=120
    )
    if not r.stdout.strip():
        print(" ERROR: 日期索引查询空结果")
        return
    print(f" {len(r.stdout)//1024}KB ({time.time()-t0:.0f}s)")
    ...
    for line in r.stdout.strip().split('\n'):
        ...
        date_close[parts[0]][parts[1].strip()] = pf(parts[2])
    alld = sorted(date_close.keys())
    print(f"  {len(alld)}交易日")
    del r
    ...
    for idx, code in enumerate(codes):
        ...
        for line in lines:
            ...
            try:
                ...
                l.append(float(parts[3])); c.append(float(parts[4])); v.append(float(parts[5]))
            except: continue
    ...
    fp = f'/tmp/alpha191_ic_v16_{time.strftime("%Y%m%d_%H%M%S")}.json'
    with open(fp, 'w') as f:
