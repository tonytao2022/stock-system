#!/usr/bin/env python3
"""
Alpha191 因子 IC v12 — 从gzip文件流式读取，零SQL依赖
======================================================
849只 × 28因子 × 5日RankIC
"""
...
def main():
    ...
    # 1. 从文件流式读取，按股票分组
    print(f"⏳ 流式读取K线数据...", end='', flush=True)
    ...
    for line in gzip.open('/tmp/all_kline.tsv.gz', 'rt'):
        ...
        code = parts[0]
        code_set.add(code)
        # [trade_date, open, high, low, close, vol] — 不带code
        stock_kline[code].append((parts[1], pf(parts[2]), pf(parts[3]),
                                  pf(parts[4]), pf(parts[5]), pf(parts[6])))
    codes = sorted(code_set)
    print(f" {len(codes)}只, {sum(len(v) for v in stock_kline.values())}条 ({time.time()-t0:.0f}s)")
    
    # 2. 构建日期索引（直接在读取时构建更高效，但这里用第二遍遍历）
    print(f"⏳ 构建日期索引...", end='', flush=True)
    date_close = defaultdict(dict)
    for code, klines in stock_kline.items():
        for row in klines:
            date_close[row[0]][code] = row[4]
    alld = sorted(date_close.keys())
    print(f" {len(alld)}交易日 ({time.time()-t0:.0f}s)")
    ...
    fp = f'/tmp/alpha191_ic_v12_{time.strftime("%Y%m%d_%H%M%S")}.json'
    with open(fp, 'w') as f:
