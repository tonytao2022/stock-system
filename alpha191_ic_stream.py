#!/usr/bin/env python3
"""
Alpha191 因子 IC — 流式加载，不一次性fetchall
===============================================
使用SSDictCursor逐行迭代，按股票分组
"""
...
def main():
    ...
    # ========== 第1轮：构建交易日-收盘价索引 ==========
    print(f"⏳ 构建收盘价索引...", end='', flush=True)
    conn = pymysql.connect(host='127.0.0.1', port=3306, user='debian-sys-maint',
                           password=pwd, database='stock_db_v2', charset='utf8mb4',
    ...
    for r in cur:
        ...
        date_close[td][r['ts_code']] = float(r['close'])
        row_cnt += 1
        if row_cnt % 100000 == 0:
            print(f" {row_cnt}K", end='', flush=True)
    cur.close(); conn.close()
    alld = sorted(date_close.keys())
    print(f" → {len(alld)}交易日, {row_cnt}条 ({time.time()-t0:.0f}s)")
    ...
    for idx, code in enumerate(codes):
        if idx % 50 == 0:
            ...
            eta = (time.time()-t2)/(idx+1)*(len(codes)-idx)/60 if idx>0 else 0
        ...
        v = np.array([float(r['vol']) for r in rows])
