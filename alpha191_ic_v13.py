#!/usr/bin/env python3
"""
Alpha191 因子 IC v13 — 单遍流式读取+立即计算
==============================================
849只 × 28因子 × 5日RankIC
"""
...
def main():
    ...
    # 1. 读取全部行
    print(f"⏳ 读取K线...", end='', flush=True)
    lines = []
    with gzip.open('/tmp/all_kline.tsv.gz', 'rt') as f:
        for line in f:
            lines.append(line.strip().split('\t'))
    print(f" {len(lines)}条 ({time.time()-t0:.0f}s)")
    ...
    for parts in lines:
        ...
        o=pf(parts[2]); h=pf(parts[3]); l=pf(parts[4]); c=pf(parts[5]); v=pf(parts[6])
    ...
    fp = f'/tmp/alpha191_ic_v13_{time.strftime("%Y%m%d_%H%M%S")}.json'
    with open(fp, 'w') as f:
