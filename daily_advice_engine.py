...
def get_top_stocks(trade_date=None, top_n=5):
    ...
    cur.execute("""
        ...
        ORDER BY ss.calibrated_score DESC, ss.composite_score DESC
        LIMIT %s
    """, (trade_date, top_n))
...
def get_holding_advice(params):
    ...
    # 读取当前持仓
    cur.execute("""
        SELECT ts_code, name, qty, cost_price, current_price, profit_pct, buy_date, status
        FROM portfolio_holdings WHERE status='HOLDING'
    """)
    
    holdings = []
    for r in cur.fetchall():
        code = r[0]; name = r[1] or ''; qty = int(r[2]); cost = float(r[3] or 0)
        cur_price = float(r[4] or 0); profit = float(r[5] or 0); buy_date = str(r[6])
        
        # 今天的评分
        cur.execute("""
            SELECT calibrated_score, composite_score FROM strategy_signal
        ...
        holdings.append({
            'ts_code': code, 'name': name, 'qty': qty, 'cost': cost,
...
def generate_report():
    ...
    if not holdings:
        ...
        for h in holdings:
            ...
            report.append(f"     持仓{h['qty']}股 | 成本¥{h['cost']:.2f} | 现价¥{h['current_price']:.2f} | 盈亏{h['profit_pct']:+.1f}%")
...
if __name__ == '__main__':
    ...
    json_path = '/opt/stock-analyzer/static/advice_data.json'
