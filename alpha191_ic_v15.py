...
def main():
    ...
    # 1. 首次遍历：收集代码列表 + 构建date_close索引
    print(f"⏳ 首次遍历: 收集股票代码 + 收盘价索引...", end='', flush=True)
    codes_set = set()
    date_close = {}
    # 先建外层的dict
    for line in gzip.open('/tmp/all_kline.tsv.gz', 'rt'):
        parts = line.strip().split('\t')
        if len(parts) < 7: continue
        code = parts[0]; td = parts[1]
        c_val = float(parts[5])
        codes_set.add(code)
        if td not in date_close:
            date_close[td] = {}
        date_close[td][code] = c_val
    ...
    for line in gzip.open('/tmp/all_kline.tsv.gz', 'rt'):
        ...
        if code != cur_code:
            ...
            if len(cur_rows) >= 30:
                ...
                o = np.array([k[1] for k in klines], dtype=np.float64)
                h = np.array([k[2] for k in klines], dtype=np.float64)
                l = np.array([k[3] for k in klines], dtype=np.float64)
                c = np.array([k[4] for k in klines], dtype=np.float64)
                v = np.array([k[5] for k in klines], dtype=np.float64)
                ...
                for td, factors in result.items():
                    ...
                    tc = date_close[td].get(cur_code)
                    fc = date_close[future_date].get(cur_code)
            ...
            if idx % 50 == 0:
                ...
                eta = (time.time()-t2)/idx*(len(codes)-idx)
                print(f"  [{idx}/{len(codes)}] {pct:.0f}% ETA{eta:.0f}s")
            
            cur_code = code
            cur_rows = []
        
        td = parts[1]
        o_val = float(parts[2]); h_val = float(parts[3])
        l_val = float(parts[4]); c_val = float(parts[5]); v_val = float(parts[6])
        cur_rows.append((td, o_val, h_val, l_val, c_val, v_val))
    
    # 最后一个code
    if len(cur_rows) >= 30:
        klines = cur_rows
        tds = [k[0] for k in klines]
        o = np.array([k[1] for k in klines]); h = np.array([k[2] for k in klines])
        l = np.array([k[3] for k in klines]); c = np.array([k[4] for k in klines])
        v = np.array([k[5] for k in klines])
        result = compute_alphas(c, o, h, l, v, tds)
        for td, factors in result.items():
            td_idx = alld.index(td) if td in alld else -1
            if td_idx < 0 or td_idx + 5 >= len(alld): continue
            future_date = alld[td_idx + 5]; tc = date_close[td].get(cur_code)
            fc = date_close[future_date].get(cur_code)
            if tc is None or fc is None or tc == 0: continue
            fwd_ret = (fc / tc - 1) * 100
            for aname, aval in factors.items():
    ...
    fp = f'/tmp/alpha191_ic_v15_{time.strftime("%Y%m%d_%H%M%S")}.json'
    with open(fp, 'w') as f:
