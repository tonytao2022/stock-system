#!/usr/bin/env python3
"""
Alpha191 因子 IC v10 — 文件管道高速加载
=========================================
849只 × 28因子 × 5日RankIC — 使用临时文件
"""

import os, sys, json, math, time, gc, subprocess, tempfile
import numpy as np
import pandas as pd
...
def run_sql_to_file(sql_text, outpath):
    """将SQL结果导出到文件（通过stdin传递SQL避免shell解释反引号）"""
    cmd = ['mysql', '-u', 'debian-sys-maint', f'-p{PWD}', 'stock_db_v2', '-N', '-B']
    with open(outpath, 'w') as f:
        r = subprocess.run(cmd, input=sql_text, stdout=f, stderr=subprocess.PIPE, text=True, timeout=120)
    if r.returncode != 0:
...
def main():
    t0 = time.time()
    tmpdir = tempfile.mkdtemp(prefix='alpha191_')
    
    # 1. 股票列表
    f = f'{tmpdir}/codes.tsv'
    n = run_sql_to_file("SELECT ts_code FROM backtest_pool", f)
    codes = [line[0].strip() for line in read_file_lines(f) if line]
    print(f"⏳ {len(codes)}只股票")
    
    # 2. 日期-收盘价索引 — 导出到文件
    print(f"⏳ 加载日期索引...", end='', flush=True)
    f = f'{tmpdir}/dates.tsv'
    n = run_sql_to_file(f"""
    ...
    for parts in read_file_lines(f):
        if len(parts) < 3: continue
        td, code = parts[0], parts[1].strip()
        date_close[td][code] = pf(parts[2])
    alld = sorted(date_close.keys())
    print(f" {len(alld)}交易日, {n}条 ({time.time()-t0:.0f}s)")
    ...
    for idx, code in enumerate(codes):
        if idx % 50 == 0:
            ...
            eta = (time.time()-t2)/(max(idx,1))*(len(codes)-idx)
            print(f"  [{idx}/{len(codes)}] {pct:.0f}% ETA{eta:.0f}s")
        
        f = f'{tmpdir}/k_{idx}.tsv'
        n = run_sql_to_file(f"""
        ...
        for p in rows:
            if len(p) < 6: continue
            tds.append(p[0])
            o.append(pf(p[1])); h.append(pf(p[2]))
            l.append(pf(p[3])); c.append(pf(p[4])); v.append(pf(p[5]))
        if len(c) < 30: continue
        
        result = compute_alphas(np.array(c), np.array(o), np.array(h),
                                np.array(l), np.array(v), tds)
    ...
    fp = f'/tmp/alpha191_ic_v10_{time.strftime("%Y%m%d_%H%M%S")}.json'
    with open(fp, 'w') as f:
