#!/usr/bin/env python3
"""
Alpha191 因子 IC 验证 v5 — 逐股SQL加载
=======================================
每只股票独立SQL查询，减少Python内存占用
847只 × 28因子 × 5日IC
"""
...
DATE_CACHE = {}  # 用于缓存全部交易日的收盘价

def build_date_cache():
    """提前构建 {date: {code: close}} 索引"""
...
def main():
    ...
    dc = build_date_cache()
    alld = sorted(dc.keys())
    print(f" {len(alld)}个交易日 ({time.time()-t0:.0f}s)")
    ...
    for idx, code in enumerate(codes):
        if idx % 50 == 0:
            print(f"  [{idx}/{len(codes)}] {time.time()-t1:.0f}s")
    ...
    print(f"  {len(codes)}只 | {len(alld)}交易日 | {START}~{END} | 耗时{elapsed:.0f}s")
    ...
    fp = f'/tmp/alpha191_ic_v5_{time.strftime("%Y%m%d_%H%M%S")}.json'
    with open(fp, 'w') as f:
