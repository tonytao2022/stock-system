...
def evaluate_strategy(trade_date, strategy):
    ...
    for ts_code, name in stocks.items():
        ...
        if holding:
            ...
            # A. 止损时间衰减（V3新规则）
            # 从DB读取sl_time_decay，格式: (5,5),(7,10),(8,999) = (跌幅%,天数上限)
            sl_time_decay_cfg = strategy.get('sl_time_decay', '')
            if sl_time_decay_cfg and isinstance(sl_time_decay_cfg, str):
                try:
                    decay_pairs = eval(sl_time_decay_cfg)
                    effective_sl_decay = 8
                    for d_sl, d_days in decay_pairs:
                        if hold_days <= d_days:
                            effective_sl_decay = d_sl
                            break
                    decay_ratio = effective_sl_decay / 100.0
                    sl_ratio = decay_ratio  # 覆盖默认sl_ratio
                except:
                    pass
            if cost > 0:
            ...
            # V3: P1退坡加延判期，评分在60-74之间低于买入线时继续持有观察
            p1_grace = int(strategy.get('p1_grace_days', 0) or 0)
            if hold_days <= 5:
                cp = 5
                ...
                if hold_days < 5:
                    passed = None
                elif p1_grace > 0 and current_score >= 60 and current_score < buy_min:
                    # 延判期内视为通过
                    passed = 1
                else:
                    passed = current_score >= p1
            elif hold_days <= 15:
                cp = 15
            ...
            results.append({
                ...
                'effective_sl_pct': round(sl_ratio * 100, 1),
                'p1_grace_days': p1_grace,
